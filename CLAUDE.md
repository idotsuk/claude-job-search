...
`scripts/triage_server.py` is the one exception to "thin": it's a small stdlib-only (`http.server`) local web app powering `/triage`'s one-at-a-time review UI. It's ephemeral (one review session, shut down from the page) and local-only (127.0.0.1) — not a pattern to reach for casually; the other scripts stay one-shot CLI tools by design.
...
/triage           # review the To Apply queue one listing at a time, decline or keep
...
A small regression test suite exists under `tests/` (plain `python3 tests/test_*.py`, no pytest framework, no CI wiring yet) — run a test file directly when touching the script it covers. It's not exhaustive (this is still skill-first, not test-first) but covers logic that's easy to silently break: `/apply` prefill field tracking, run-report HTML escaping, the shared status enum, and `/triage`'s decision/logging logic. Beyond that, there's no build/lint suite; the operative "commands" are the maintenance scripts the skills call:
...
python3 scripts/triage_server.py [--port N] [--no-browser]                  # one-at-a-time review UI for the To Apply queue (/triage)
python3 scripts/triage_suggestions.py [--mark-seen ID ...]                  # dump/tag data/decline-log.yaml entries for job-search step 0b
...
# tests (plain unittest, run individually — no pytest/tox config):
python3 tests/test_triage_server.py
python3 tests/test_triage_suggestions.py
python3 tests/test_apply_driver.py
python3 tests/test_report_html_escaping.py
python3 tests/test_status_order_sync.py
```
...
### Six skills, one shared data model

- **`/job-search`** (`.claude/skills/job-search/SKILL.md`) — the sweep. Reads `config.yaml`, loads `data/sources.yaml` (the source registry) and `data/listings/*.md`, searches LinkedIn + company boards + discovered sources in parallel, dedupes, writes new listing files, syncs Gmail/WhatsApp/Calendar, updates the registry, writes a run report + chart. Step 0b reads `data/decline-log.yaml` before each sweep's scan and asks about not-yet-`suggested` declines — see "Decline log" below.
- **`/triage`** (`.claude/skills/triage/SKILL.md`) — launches `scripts/triage_server.py`, a local one-at-a-time review UI over the `To Apply` queue. Each listing is kept (to apply now, or flagged to reconsider later) or declined with a categorized reason. Decisions write back to the listing file's frontmatter and to `data/decline-log.yaml`.
...
- **`/network-scan`** (`.claude/skills/network-scan/SKILL.md`) — occasional (e.g. monthly), separate from the regular sweep. Cross-references LinkedIn contacts' current employers against listings `/job-search` already found, and flags `warm_contact: true` on any match. Does not discover jobs itself — that's `/job-search`'s job alone; this skill never visits a careers page or writes a new listing.
...
- `data/decline-log.yaml` — `/triage`'s structured decision history, one entry per declined listing (`id`, `company`, `role`, `reason`, optional `note`, `suggested: true` once asked about). `job-search/SKILL.md` step 0b (`scripts/triage_suggestions.py` for the mechanical read/mark-seen parts) reads it *before* each sweep's scan and asks about every not-yet-`suggested` decline interactively — a single decline is enough to ask about, it doesn't wait for a repeat — applying an accepted suggestion to `config.yaml` (`company_blocklist`, `anti_interests`, or `location_blocklist`) in time to filter that same run. This is advisory-only: nothing auto-edits `config.yaml` without an explicit yes, and a "no" is remembered (via `suggested: true`) so that exact decline isn't re-asked — a different, new decline still gets its own ask.
...
- Treat `.claude/skills/*/SKILL.md` as the spec. If you change scraping/dedup/registry/status/scoring/tailoring/triage logic, update the corresponding numbered step in the skill doc in the same change — the prose there is what an agent actually executes at runtime.
