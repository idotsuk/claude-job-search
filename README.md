...
- **`/triage`** — a local, one-at-a-time review UI for the "To Apply" pile: keep a listing to apply now or flag it to reconsider later, or decline it with a reason (company fit, role fit, tech-stack gap, other) and an optional note. Every decision writes back to the listing and to a decision log for `/job-search` to learn from on its next run.
...
Run `/triage` to work through the "To Apply" pile one card at a time in a local browser tab (`scripts/triage_server.py`, no extra install — stdlib only). **Apply** leaves a listing queued and ready; **Reconsider later** also leaves it queued but flags it (`keep_intent: reconsider`) so `/apply` gives it a second look instead of working it at face value; **Decline** demotes it to `status: Skipped` with a categorized reason and, for anything but company fit, a required note on what's missing. Prev/Next let you browse and revise freely; **Review** shows everything decided so far, grouped by outcome; **Finish for now** ends the session anytime, progress saved.
